...
lv_obj_t * switch_create(lv_obj_t * parent)
{
    ...
    static lv_style_t style_base_checked;
    ...
    if (!style_inited) {
        ...
        if (dual_screen_check_target(DUAL_SCREEN_TARGET_ALL)) {
            ...
            lv_style_set_width(&style_base, 90);
            lv_style_set_height(&style_base, 50);
            lv_style_set_bg_color(&style_base, lv_color_hex(0xf9f9f9));

            lv_style_init(&style_base_checked);
            lv_style_set_bg_color(&style_base_checked, lv_color_hex(0x2b55ff));

            lv_style_init(&style_knob);
            lv_style_set_bg_color(&style_knob, lv_color_hex(0x2b55ff));

            lv_style_init(&style_knob_checked);
            lv_style_set_bg_color(&style_knob_checked, lv_color_hex(0xffffff));
    ...
    if (dual_screen_check_target(DUAL_SCREEN_TARGET_ALL)) {
        ...
        lv_obj_add_style(lv_switch_0, &style_base_checked, LV_PART_INDICATOR | LV_STATE_CHECKED);
